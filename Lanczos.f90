...
contains
  ...
  subroutine LanczosGroundState(NS, NDet, MatVec, maxit, tol, E0, iters, resid)
    implicit none
    ...
    real(kind=pr), allocatable :: q(:), qprev(:), w(:)
    real(kind=pr), allocatable :: d(:), e(:)
    ...
    E0 = Enew
    if (present(iters)) iters = k
    if (present(resid)) resid = diff

    deallocate(q, qprev, w, d, e)
  ...
  function LowestEigen_Tridiag(d_in, e_in, m, info) result(Emin)
    ...
    !---------------------------------------------------------
    ! LAPACK DSTEV is DOUBLE PRECISION. Declare interface as such.
    !---------------------------------------------------------
    interface
